...
namespace MobilePayService.Methods
{
    public class DBManager
    {
        ...
        public static void InsertRecord(BCClientModel data, SqlConnection conn)
        {
            ...
            string query = "if exists(select 1 from MobilePayOnBoarding   where  [TenantIdFormatted] = '" + data.extractedTenantId + "' and [BCTenantId] = '" + data.BCTenantId + "'and [AzClientId]'" + data.AzTenantId + "'and [AzClientSecret]'"+data.AzClientSecret+ "'and [AzTenantId]'"+data.AzTenantId+"') begin UPDATE MobilePayOnBoarding SET UserName = '" + data.userName + "', Password = '" + data.password + "' ,BCTenantId = '" + data.BCTenantId + "', State = '" + data.state + "' , CodeVerifier = '" + data.code_verifier + "', CodeChallenge = '" + data.code_challenge + "', Premium='" + data.enableCallback + "' where TenantIdFormatted = '" + data.extractedTenantId + "' and [BCTenantId] = '" + data.BCTenantId + "' end else begin  insert into MobilePayOnBoarding (UserName,Password,BCTenantId,State,CodeVerifier,CodeChallenge,TenantIdFormatted,Premium,AzClientId,AzClientSecret,AzTenantId) values('" + data.userName + "','" + data.password + "','" + data.BCTenantId + "','" + data.state + "','" + data.code_verifier + "','" + data.code_challenge + "','" + data.extractedTenantId + "','" + data.enableCallback + "','" + data.AzClientId + "','" + data.AzClientSecret + "','" + data.AzTenantId + "') end";
